...
public class GoogleHandler {
    ...
    //sheets
    private static ArrayList<File> spreadsheets = new ArrayList<>();
    private static Dictionary<Integer,ArrayList<Sheet>> sheetsOfSpreadsheet = new Hashtable<>();
    ...
    public static void applyRequests(ArrayList<Request> requests, Integer spreadsheetID) throws IOException {

            sheetService.spreadsheets().batchUpdate(String.valueOf(spreadsheetID),
    ...
    public static void start() throws GeneralSecurityException, IOException {
       ...
       NetHttpTransport HTTP_TRANSPORT = GoogleNetHttpTransport.newTrustedTransport();
        ...
        for (File file: spreadsheets){
            int sheetID = Integer.parseInt(file.getId());
            ArrayList<Sheet> sheets = downloadAllSheets(file.getId());
            sheetsOfSpreadsheet.put(sheetID,sheets);
        }
    }

    public static List<List<Object>> getSheet(Integer spreadsheetId, String range) throws IOException {
        ValueRange response = sheetService.spreadsheets().values()
                .get(String.valueOf(spreadsheetId), range)
                .execute();
